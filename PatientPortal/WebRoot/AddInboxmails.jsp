...
<script type="text/javascript" src="<%=request.getContextPath() + "/js/image.js"%>"> </script>
	...
	<body>
		...
		<table>
			...
			<form action="./AddInboxmailsAction" method="post" name="inboxmails"
				onsubmit='return validate()'>
				<center>
					<h2>
						<font color='#sharew'>Compose Mail</font>
					</h2>
				</center>
				<pre>
<table align='center'>
        	...
        	<tr>
                ...
                <td align='right'>
                        <select name='tomailidref'
									onChange='cleartext()' style="width: 69%;">
                               <option value='select' selected='true'>
                               <font size='3' face='Verdana'>Select </font>
                               </option>
                              <%
                              	List<UserdetailsTO> listuserdetails = new UserdetailsDaoImpl().viewUserdetails();

                              	for (UserdetailsTO userdetailsTO : listuserdetails) {
                              %>
	<option value='<%=userdetailsTO.getUserid()%>'>
                               <font size='3' face='Verdana'><%=userdetailsTO.getEmail()%></font>
                               </option>
									<%
										}
									%>
        	...
        	<tr>
                ...
                <td align='right'>
                            <textarea
									style="height: 50px; width: 173px;" value='' name='mailbody'></textarea>
                </td>
            </tr>
			        <tr align='center'>
                          <td></td>
...
<input type='hidden' name='userid'
						value='<%=session.getAttribute("useridref")%>'>
</pre>
			</form>
			<script language='JavaScript' type='text/javascript'>
