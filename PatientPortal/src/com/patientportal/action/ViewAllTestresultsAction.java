...
public class ViewAllTestresultsAction extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		List<TestresultsTO> testresultstolist = null;
		String path = "";
		try {
			
			HttpSession session = request.getSession();
			if(session.getAttribute("role").equals("patient")){
				testresultstolist = new TestresultsServiceImpl()
			.getPatientTestresults((Integer)session
					.getAttribute("userid"));
			}else{
				testresultstolist = new TestresultsServiceImpl()
				.getPatientTestresults(Integer.parseInt((String)session.getAttribute("userdata")));
			}
			
			if (testresultstolist != null) {
				request.setAttribute("testresultstolist", testresultstolist);
				path = UtilConstants._VIEW_TESTRESULTS_JSP;
			} else {
