...
public class AddUserdetailsAction extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			flag = new UserdetailsServiceImpl().addUserdetails(userdetailsto);


			if (flag) {
				request.setAttribute("status", UtilConstants._ADD_USERDETAILS);
				if("admin".equals(request.getSession().getAttribute("role")))
					path = "./AddDoctor.jsp";
				else
					path = UtilConstants._ADD_USERDETAILS_JSP;
				
			} else {
				request.setAttribute(UtilConstants._STATUS,
						UtilConstants._ADD_USERDETAILS_FAILED);
			...
			e.printStackTrace();
			request.setAttribute(UtilConstants._STATUS, UtilConstants._ADD_USERDETAILS_FAILED);
			path = UtilConstants._STATUS_PATH;
		}finally{
